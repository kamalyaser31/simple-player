...
from .components import CancelFlag, CancelledError, add_to_path, has_all, recovery_message
...
from .state import clear_ses, get_ses, mk_ses, ses_idx, set_now, set_ses, sync_removed_sources
...
def open_yt(ctx):
    if ctx.frame is None:
        return
    if not _components_ready_modal(ctx):
        return
...
def search_yt(ctx):
    if ctx.frame is None:
        return
    if not _components_ready_spoken(ctx):
        return
...
def on_esc(ctx):
    ...
    ctx.player.stop()
    removed = set(ses.get("urls", []))
    ctx.player.remove_sources(removed)
    sync_removed_sources(ses, removed)
...
def try_next(ctx):
    ...
    ses["pending_next"] = {"from": cur_idx, "to": nxt, "key": key, "source": str(item.url or "")}
    prep = _cache_get(key)
...
def on_playback_finished(ctx):
    ses = get_ses(ctx)
    if ses is None:
        return False
    src = str(ctx.player.current_source or "").strip()
    if not src or src not in ses.get("urls", set()):
        return False
    moved = try_next(ctx)
    if moved is True:
        return True
...
def _load_next_bg(ctx, ses, item, from_idx, to_idx):
    ...
    def done(_future):
        ...
        if not isinstance(prep, dict):
            wx.CallAfter(_next_failed, ctx, ses, key)
            return
        _cache_set(key, prep)
...
def _next_failed(ctx, ses, key):
    cur = get_ses(ctx)
    if cur is None or cur is not ses:
        return
    pending = ses.get("pending_next")
    if isinstance(pending, dict) and str(pending.get("key") or "") == str(key or ""):
        ses["pending_next"] = None
    ctx.speak(
        _("Could not prepare the next YouTube video."),
        _("Next YouTube video unavailable."),
    )
...
def _ensure_queued(ctx, ses, item, prep):
    ...
    if not key:
        return False
    if _is_queued(ses, key) and _player_has_source(ctx.player, key):
        return True
    ses.setdefault("queued", set()).discard(key)
...
def _player_has_source(player, source):
    if hasattr(player, "has_source"):
        try:
            return bool(player.has_source(source))
        except Exception:
            return False
    return True
...
def _components_ready_modal(ctx):
    if has_all():
        return True
    message = recovery_message() or _("YouTube components are missing.")
    _show_error(ctx, message, _("YouTube"))
    return False


def _components_ready_spoken(ctx):
    if has_all():
        return True
    message = recovery_message() or _("YouTube components are missing.")
    ctx.speak(message, _("YouTube components need repair."))
    return False

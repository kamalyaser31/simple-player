...
HELPER_READY = "ready"
HELPER_BROKEN = "broken"
HELPER_REPAIR_REQUIRED = "repair-required"
...
def has_yt():
    return yt_readiness()["state"] == HELPER_READY


def has_deno():
    return deno_readiness()["state"] == HELPER_READY
...
def yt_readiness(timeout=2):
    return _file_readiness("yt-dlp", YT_EXE, timeout=timeout, validate=False)


def deno_readiness(timeout=2):
    return _file_readiness("Deno", DENO_EXE, timeout=timeout, validate=True)
...
def recovery_message():
    states = helper_readiness()
    bad = [name for name, data in states.items() if data.get("state") != HELPER_READY]
    if not bad:
        return ""
    return (
        "YouTube components are missing or need repair: "
        + ", ".join(bad)
        + ". Install or repair YouTube components from Settings before using YouTube features."
    )
...
def _file_readiness(name, path, timeout=5, validate=False):
    if not os.path.isfile(path):
        return {"name": name, "state": HELPER_MISSING, "path": path, "message": "missing"}
    try:
        if os.path.getsize(path) <= 0:
            return {"name": name, "state": HELPER_REPAIR_REQUIRED, "path": path, "message": "empty file"}
    except OSError:
        return {"name": name, "state": HELPER_BROKEN, "path": path, "message": "not readable"}
    if not validate:
        return {"name": name, "state": HELPER_READY, "path": path, "message": "ready"}
    try:
        proc = subprocess.run(
            [path, "--version"],
            stdout=subprocess.PIPE,
        ...
        return {"name": name, "state": HELPER_BROKEN, "path": path, "message": str(exc)}
    if proc.returncode == 0:
        return {"name": name, "state": HELPER_READY, "path": path, "message": "ready"}
    message = (proc.stderr or proc.stdout or "validation failed").strip()
    return {"name": name, "state": HELPER_BROKEN, "path": path, "message": message}


def _cleanup_deno_temps():
    try:
        names = os.listdir(APP_DIR)
    except OSError:
        return
    for name in names:
        if not name.startswith("sap_deno_") or not name.endswith(".tmp"):
            continue
